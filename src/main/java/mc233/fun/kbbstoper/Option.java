...
public enum Option {
	...
	BBS_URL("bbs.url"), BBS_PAGESIZE("bbs.pagesize"),
	...
	public static void load() {
		if (file == null) {
			file = new File(KBBSToper.getInstance().getDataFolder(), "config.yml");
		}
		config = YamlConfiguration.loadConfiguration(file);// 用这个方法加载配置可以解决编码问题
		try (Reader reader = new InputStreamReader(KBBSToper.getInstance().getResource("config.yml"),
				StandardCharsets.UTF_8)) {// 读取默认配置
			YamlConfiguration defConfig = YamlConfiguration.loadConfiguration(reader);
			config.setDefaults(defConfig);// 设置默认
		} catch (IOException ioe) {
			KBBSToper.getInstance().getLogger().log(Level.SEVERE, "读取默认配置文件时出错!", ioe);
		}
	}
