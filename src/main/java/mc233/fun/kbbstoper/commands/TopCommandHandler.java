...
public class TopCommandHandler implements CommandHandler {
    ...
    public void handle(CommandSender sender, String[] args) {
        ...
        List<Poster> all = SQLManager.getSQLer().getTopPosters();
        all.addAll(SQLManager.getSQLer().getNoCountPosters());
        int size=all.size();
        ...
        if (start==end) lines.add(Message.NOPLAYER.getString());
        lines.add(
                Message.PREFIX.getString()+
                        Message.PAGEINFOTOP.getString()
