...
public class CLI implements TabExecutor {
		...
		new BukkitRunnable() {
			...
			public void task() {
				...
				case "binding": {
					...
					if (args.length == 2) {
						...
						if (poster != null) {
							...
							long settedcd = Option.BBS_CHANGEIDCOOLDOWN.getInt() * (long) 86400000;// 设置的cd
				...
				case "list": {
					...
					int totalpage = (int) Math.ceil((double) crawler.ID.size() / Option.BBS_PAGESIZE.getInt());
					if (page > totalpage) {
					...
					for (int i = (page - 1) * Option.BBS_PAGESIZE.getInt(); i < page
							* Option.BBS_PAGESIZE.getInt(); i++) {
				...
				case "top": {
					...
					int totalpage = (int) Math.ceil((double) posterlist.size() / Option.BBS_PAGESIZE.getInt());
					if (page > totalpage) {
						sender.sendMessage(Message.PREFIX.getString() + Message.OVERPAGE.getString());
						return;
					}
				...
				case "check": {
					...
					case "player": {
						...
						String bbsname = poster.getBbsname();
						sender.sendMessage(
								Message.PREFIX.getString() + Message.OWNERID.getString().replaceAll("%ID%", bbsname));
						return;
					}
					}
				}
	...
	public double getQueryCooldown(UUID uuid) {
		int cooldown = Option.BBS_QUERYCOOLDOWN.getInt() * 1000;
